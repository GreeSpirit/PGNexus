...
export async function GET(request: NextRequest) {
  ...
  if (jobidStr && !filename) {
    ...
    try {
      ...
      return NextResponse.json({ content, title, frontmatter: {}, source: 'db' });
    } catch (err) {
      console.error('Error fetching daily update from DB:', err);
      return NextResponse.json({ error: 'Failed to read daily update' }, { status: 500 });
    }
  }
  ...
  if (!filename) {
    return NextResponse.json(
      { error: 'filename or jobid parameter is required' },
      { status: 400 }
    );
  }
  ...
  try {
    const fileContent = await readFile(join(contentDir, fileToLoad), 'utf-8');
    const { data: frontmatter, content } = matter(fileContent);
    return NextResponse.json({ filename: fileToLoad, content, frontmatter, title: '', source: 'file' });
  } catch {
